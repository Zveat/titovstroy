...
export function loadBrand({ force = false } = {}) {
  ...
  _inflight = (async () => {
    try {
      ...
      //
      // НО «загружено» при этом НЕ ставим. Экран входа читает оформление, когда
      // человек ещё не вошёл, и правила такое чтение отклоняют; если бы мы
      // запомнили неудачу как результат, приложение так и осталось бы с
      // умолчаниями до конца сессии — уже войдя и имея полное право читать.
      _loaded = false;
    }
    _inflight = null;
    applyDocumentBrand(_brand);
