...
function MainApp({ currentUser, setCurrentUser, editorTab, takeoverEditLease }) {
  ...
  const openAvrBuilder = (obj, est) => {
    // Помечаем то, что уже уходило в прежние акты этого объекта. ТОЛЬКО подпись: галочки и
    // количества не трогаем — иначе на втором и третьем акте приходится поднимать первый и
    // вспоминать, что уже сдавал.
      ...
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        /* ── КНОПКИ ОДНОГО РЯДА — ОДНОГО РАЗМЕРА ─────────────────────────────
           Размеры набирались по месту, и в одном ряду стояли кнопки разной высоты
           и очень разной длины: замерено на экране 390px — «🚪 Выйти» 77x24,
           «☁ Обновить» 94x24, «⚠ N требуют внимания» 153x24, а в панели объектов
           рядом 31, 33, 34 и 36 по высоте. Ряд от этого выглядит собранным наспех.
           .tap-even — панель кнопок: выравниваем и высоту, и ширину. Ширину даём
           через min-width, а не растягиванием на всю строку: кнопка догоняет
           соседей, но длинную подпись не обрезает и лишнего места не занимает.
           .tap-row  — чипы фильтров: там выравниваем ТОЛЬКО высоту. Подписи у них
           от «Все» до «Согласование сметы», и одинаковая ширина превратила бы
           девять чипов в пять рядов пустоты.
           На телефоне высота 44px: палец накрывает примерно столько, а в 24px
           попадаешь через раз — промах по «Выйти» стоит дороже всего. */
        .tap-even button,.tap-row button{min-height:36px}
        .tap-even button{min-width:155px;justify-content:center}
        ...
        @media(max-width:700px){
          ...
          /* Под палец — 44px (правило и замеры выше, у .tap-even). Высоту добираем
             отступами: сам текст не растёт, поэтому вёрстка вокруг не съезжает.
             Ширина остаётся общей, 155px: две такие кнопки в строку на 390px
             помещаются, а на экране поуже ряд просто переносится — кнопки при
             этом остаются одного размера, что и требовалось. */
          .tap-even button,.tap-row button{min-height:44px!important;padding-top:8px!important;padding-bottom:8px!important}
          /* НИЖНЕЕ МЕНЮ ПРЯЧЕТСЯ, ПОКА ПЕЧАТАЮТ.
             Меню закреплено снизу (position:fixed; bottom:0). Когда на айфоне
             вылезает клавиатура, Safari ужимает видимую часть страницы, но
             закреплённые элементы продолжает считать по ПОЛНОЙ высоте окна —
             и меню оказывается посреди экрана, поверх формы. Поймано на боевом:
             панель «Главная · Объекты · Аналитика · Финансы · Ещё» висела ровно
             посередине над клавиатурой. Раньше это скрадывал автозум, а после
             запрета масштабирования (maximum-scale=1 в index.html) вылезло наружу.
             Пока в поле стоит курсор, меню не нужно — прячем. Отпустил поле —
             вернулось. Заодно на экране больше места под саму форму.
             Флажки и переключатели не в счёт: от них клавиатуры нет, а меню
             пропадало бы от простого нажатия галочки.
             Списки (select) тоже не в счёт: фокус остаётся на них и после
             закрытия колеса выбора, и меню не вернулось бы до нажатия мимо. */
          body:has(input:not([type=checkbox]):not([type=radio]):focus, textarea:focus) .mob-nav-wrap{display:none!important}
        ...
        <div className="page" style={{background:"#f1f5f9",minHeight:"var(--vh100,100vh)",paddingBottom:40,maxWidth:1600}}>
          ...
          <div className="hero" style={{background:"linear-gradient(135deg,#0f172a 0%,#1e293b 70%,#283549 100%)",borderRadius:16,padding:"28px 32px",marginBottom:24,position:"relative",overflow:"hidden",boxShadow:"0 4px 20px rgba(15,23,42,.3)"}}>
            ...
            <div style={{position:"relative",zIndex:1,display:"flex",alignItems:"flex-start",justifyContent:"space-between",flexWrap:"wrap",gap:12}}>
              ...
              <div className="tap-even" style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
        ...
        <div style={{padding:"20px 16px 90px",minHeight:"var(--vh100,100vh)"}}>
          ...
          {objectTab==="list" && (
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              ...
              <div className="tap-even" style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
              ...
              {/* Фильтр по статусу */}
              <div className="tap-row" style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                              ...
                              <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:6,flexShrink:0}}>
                                ...
                                <div style={{display:"flex",gap:4}}>
                                  ...
                                  {accessAllows(currentPermissions.documentEdit, estimatorObjectIds.has(obj.id)) && (
                                    <button title="Редактировать акт" onClick={()=>{ setAvrSearch(""); setAvrReqOpen(!_narrowScreen()); setAvrModal({ ...r, lines: markCoveredLines((r.lines||[]).map(l=>({...l,included:true,doneQty:l.doneQty})), avrCoverage(reportsRef.current, r.objectId, { excludeId: r.id })) }); }}
