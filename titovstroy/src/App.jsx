...
// ─── FIREBASE CONFIG ──────────────────────────────────────────────────────────
// ВАЖНО: замените на ваши данные из Firebase Console
// https://console.firebase.google.com → создайте проект → Web app → скопируйте config
const firebaseConfig = {
  apiKey:            "ЗАМЕНИТЕ_НА_ВАШ_API_KEY",
  authDomain:        "ЗАМЕНИТЕ.firebaseapp.com",
  databaseURL:       "https://ЗАМЕНИТЕ-default-rtdb.firebaseio.com",
  projectId:         "ЗАМЕНИТЕ",
  storageBucket:     "ЗАМЕНИТЕ.appspot.com",
  messagingSenderId: "ЗАМЕНИТЕ",
  appId:             "ЗАМЕНИТЕ"
};
...
// ─── ХРАНИЛИЩЕ ───────────────────────────────────────────────────────────────
// Приоритет: Firebase Realtime DB → localStorage → память сессии
const _mem = {};
const storage = {
  async get(key) {
    // 1. Firebase (общая база для всех устройств)
    try {
      if (firebaseDb) {
  ...
  async set(key, value) {
    ...
    // 1. Firebase
    try {
      if (firebaseDb) await set(ref(firebaseDb, key), parsed);
    } catch(e) {}
    // 2. localStorage
...
function LoginScreen({ onLogin }) {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      else await storage.set(USERS_KEY, JSON.stringify(DEFAULT_USERS));
    ...
    } catch(e) { setError("Ошибка соединения, попробуйте снова"); }
    setLoading(false);
  };
    ...
    <div style={{fontFamily:"'Golos Text','Segoe UI',sans-serif",background:"#0c0e1a",minHeight:"100vh",color:"#ddd8ce"}}>
      ...
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .wrow{display:grid;align-items:start;padding:9px 14px;border-radius:7px;gap:8px;transition:background .12s}
        ...
        <div>
          ...
          <div style={{maxWidth:1160,margin:"0 auto",padding:"18px 18px"}}>
            ...
            <div className="card up" style={{padding:"16px 20px",marginBottom:16}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"2fr 1.4fr 0.8fr 1.4fr 1.4fr 2fr",gap:10}}>
      ...
      ═══════════════════════════════════════════════════════════════════ */}
      {showKP&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.78)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:300,padding:16}}
          onClick={()=>setShowKP(false)}>
          <div style={{background:"#f5f2ec",color:"#1a1a28",borderRadius:14,padding:"30px 34px",maxWidth:640,width:"100%",maxHeight:"88vh",overflowY:"auto"}}
            onClick={e=>e.stopPropagation()}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:16}}>
              <div>
                <div style={{fontWeight:900,fontSize:19}}>Ценовое предложение</div>
                <div style={{fontSize:11,color:"#888",marginTop:2}}>на услуги ремонта и отделки недвижимости</div>
              </div>
              <div style={{textAlign:"right"}}>
                <div style={{fontWeight:800,fontSize:14,color:"#b8904a"}}>TitovStroy</div>
                <div style={{fontSize:10,color:"#888"}}>БИН 231040002769</div>
                <div style={{fontSize:10,color:"#888"}}>WA: +7 707 982 4915</div>
              </div>
            </div>
            <div style={{background:"#e8e4da",borderRadius:8,padding:"11px 14px",marginBottom:16,fontSize:12,display:"grid",gridTemplateColumns:"1fr 1fr",gap:"4px 14px"}}>
              ...
              <thead>
                <tr style={{background:"#1a1a28",color:"#f5f2ec"}}>
                  {["№","Раздел","Наименование","Ед.","Объём","Слож.","Цена","Сумма"].map(h=>(
                    <th key={h} style={{padding:"7px 7px",textAlign:["№","Ед.","Объём"].includes(h)?"center":"left",fontSize:11,fontWeight:600}}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {kpItems.map((item,i)=>(
                  <tr key={i} style={{background:i%2===0?"#f5f2ec":"#e8e4da"}}>
                    <td style={{padding:"6px 7px",textAlign:"center",color:"#999"}}>{i+1}</td>
                    <td style={{padding:"6px 7px",color:"#606",fontSize:11}}>{item.sub}</td>
                    <td style={{padding:"6px 7px",fontWeight:500}}>{item.name}</td>
                    <td style={{padding:"6px 7px",textAlign:"center",color:"#888"}}>{item.unit}</td>
                    <td style={{padding:"6px 7px",textAlign:"center"}}>{item.qty}</td>
                    <td style={{padding:"6px 7px",fontSize:10,color:"#888"}}>{COMPLEXITY.find(c=>c.key===item.cpx)?.label.split(" ")[0]}</td>
                    <td style={{padding:"6px 7px",textAlign:"right"}}>{fmt(item.price)} ₸</td>
                    <td style={{padding:"6px 7px",textAlign:"right",fontWeight:700}}>{fmt(item.total)} ₸</td>
                  </tr>
                ))}
              </tbody>
            </table>
            <div style={{background:"#1a1a28",borderRadius:8,padding:"12px 16px",color:"#f5f2ec",marginBottom:14}}>
              {discount>0&&<div style={{display:"flex",justifyContent:"space-between",fontSize:12,color:"#d07070",marginBottom:5}}><span>Скидка {discount}%</span><span>− {fmt(discAmt)} ₸</span></div>}
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <span style={{fontSize:13}}>ИТОГО:</span>
                <span style={{fontSize:26,fontWeight:900,color:"#b8904a"}}>{fmt(final)} ₸</span>
              </div>
              {proj.area&&<div style={{textAlign:"right",fontSize:10,color:"#666",marginTop:2}}>{fmt(final/Number(proj.area))} ₸/м²</div>}
            </div>
            {note&&<div style={{background:"#e8e4da",borderRadius:8,padding:"9px 13px",fontSize:12,color:"#555",marginBottom:14}}>{note}</div>}
            <div style={{fontSize:11,color:"#444",lineHeight:1.8,marginBottom:18,background:"#ece8da",borderRadius:8,padding:"14px 16px"}}>
              <div style={{fontWeight:700,color:"#1a1a28",marginBottom:8,fontSize:12}}>Условия:</div>
              {[
                ...
                <div key={i} style={{display:"flex",gap:8,marginBottom:4}}>
                  <span style={{color:"#b8904a",fontWeight:700,minWidth:16}}>{i+1}.</span>
                  <span>{text}</span>
                </div>
              ))}
              <div style={{display:"flex",gap:8,marginTop:4}}>
                <span style={{color:"#b8904a",fontWeight:700,minWidth:16}}>7.</span>
