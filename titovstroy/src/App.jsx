...
const storage = {
  async get(key) {
    // 1. Firebase с таймаутом 3 сек
    try {
      if (firebaseDb) {
        const snapshot = await withTimeout(get(ref(firebaseDb, key)), 3000);
  ...
  async set(key, value) {
    ...
    // 1. Firebase с таймаутом 3 сек
    try {
      if (firebaseDb) await withTimeout(set(ref(firebaseDb, key), parsed), 3000);
    } catch(e) {}
    // 2. localStorage
