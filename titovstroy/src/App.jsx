            ...
            {localPrices && <>
              ...
              <div style={{paddingTop:10,borderTop:"1px solid #e5e7eb",marginTop:6}}>
                ...
                <button onClick={savePrices} disabled={priceSaving}
                  style={{width:"100%",background:"#2563eb",color:"#f3f4f6",border:"none",borderRadius:8,padding:"11px",fontFamily:"inherit",fontSize:13,fontWeight:800,cursor:"pointer"}}>
...
function KPContent({ proj, kpItems, discount, discAmt, final, note }) {
  ...
  return (
    <div style={{fontFamily:"'Inter','Segoe UI',sans-serif",color:"#111827",background:"#ffffff"}}>
      ...
      {/* Блок клиента */}
      <div style={{background:"#e5e7eb",borderRadius:10,padding:"13px 16px",marginBottom:16,display:"grid",gridTemplateColumns:"1fr 1fr",gap:"5px 20px",fontSize:13}}>
        {[["Заказчик",proj.name||"—"],["Телефон",proj.phone||"—"],["Объект",proj.type||"—"],["Адрес",proj.address||"—"],["Дата расчёта",today()],["Действует до",validUntil()],["Менеджер",proj.manager||"—"]].map(([k,v])=>(
          <div key={k}><span style={{color:"#9ca3af"}}>{k}: </span><strong>{v}</strong></div>
        ))}
      </div>
      ...
      {(() => {
        ...
        return (
          <div style={{marginBottom:14}}>
              ...
              return (
                <div key={cat} style={{marginBottom:12}}>
                  ...
                  <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
                    <thead>
                      <tr style={{background:"#e5e7eb",color:"#9ca3af"}}>
                    ...
                    <tbody>
                      {items.map((item,i) => {
                        rowNum++;
                        return (
                          <tr key={i} style={{background:i%2===0?"#ffffff":"#f3f4f6",borderBottom:"1px solid #e5e7eb"}}>
                            <td style={{padding:"6px 8px",textAlign:"center",color:"#9ca3af",fontSize:11}}>{rowNum}</td>
                            <td style={{padding:"6px 8px",color:"#9ca3af",fontSize:11,fontWeight:500}}>{item.sub}</td>
                            <td style={{padding:"6px 8px",fontWeight:600,fontSize:12}}>{item.name}</td>
                            <td style={{padding:"6px 8px",textAlign:"center",color:"#9ca3af",fontSize:11}}>{item.unit}</td>
                            <td style={{padding:"6px 8px",textAlign:"center",fontWeight:500}}>{item.qty}</td>
                            <td style={{padding:"6px 8px",fontSize:11,color:"#9ca3af"}}>{COMPLEXITY.find(c=>c.key===item.cpx)?.label.split(" ")[0]||"Стандарт"}</td>
                            <td style={{padding:"6px 8px",textAlign:"right",color:"#9ca3af"}}>{fmt(item.price)} ₸</td>
            ...
            {/* Итоговая сводка */}
            <div style={{background:"#e5e7eb",borderRadius:8,padding:"12px 16px",marginTop:8,marginBottom:4}}>
              <div style={{fontWeight:700,fontSize:12,color:"#374151",marginBottom:8,textTransform:"uppercase",letterSpacing:.5}}>Сводка по разделам</div>
              {catOrder.map(cat => (
                <div key={cat} style={{display:"flex",justifyContent:"space-between",fontSize:13,marginBottom:5,paddingBottom:5,borderBottom:"1px solid #e5e7eb"}}>
      ...
      {/* Примечание */}
      {note&&<div style={{background:"#e5e7eb",borderRadius:8,padding:"10px 14px",fontSize:12,color:"#9ca3af",marginBottom:14}}>{note}</div>}
      ...
      <div style={{background:"#e5e7eb",borderRadius:10,padding:"14px 18px",fontSize:12,color:"#374151",lineHeight:1.75,marginBottom:20}}>
        ...
        <div style={{display:"flex",gap:10,marginTop:5}}>
          <span style={{color:"#9ca3af",fontWeight:500,minWidth:18,flexShrink:0}}>7.</span>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:24,marginTop:16}}>
        <div>
          <div style={{fontSize:11,color:"#9ca3af",marginBottom:4}}>Заказчик</div>
          <div style={{borderTop:"1px solid #d1d5db",paddingTop:8,marginTop:32}}/>
          <div style={{fontSize:11,color:"#9ca3af"}}>{proj.name||"________________________________"}</div>
          <div style={{fontSize:10,color:"#9ca3af",marginTop:2}}>М.П.</div>
        </div>
      ...
      {loading ? (
        ...
        <div style={{paddingBottom:90}}>
          ...
          <div style={{position:"fixed",bottom:0,left:"220px",right:0,background:"#f3f4f6",borderTop:"1px solid #e5e7eb",padding:"12px 24px",zIndex:20}}>
            ...
            <button onClick={savePrices} disabled={priceSaving}
              style={{width:"100%",display:"block",background:"#2563eb",color:"#f3f4f6",border:"none",borderRadius:9,padding:"13px",fontFamily:"inherit",fontSize:14,fontWeight:800,cursor:"pointer"}}>
              {priceSaving ? "💾 Сохранение..." : "💾 Сохранить прайс"}
            </button>
          </div>
        </div>
