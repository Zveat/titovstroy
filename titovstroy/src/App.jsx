  ...
  const generateContractDocx = async (c, client, ca) => {
    ...
    if(type==="repair_fiz"||type==="repair_yur"){
      children = [
        ...
        P([T("2.1. По настоящему Договору Подрядчик обязуется выполнить комплекс ремонтно-отделочных работ, установленный в Приложении \u21161, а Заказчик обязуется принять результат и оплатить в соответствии со ст. 651 ГК РК.")]),
        P([T("2. СТОИМОСТЬ И ПОРЯДОК РАСЧЁТОВ",{b:true})]),
        ...
        P([T("3. СРОКИ ВЫПОЛНЕНИЯ РАБОТ",{b:true})]),
        P([T("5.1. Подрядчик обязан выполнить Работы в соответствии с Приложением \u21161.")]),
        P([T("4. ГАРАНТИИ",{b:true})]),
        P([T("8.1. Гарантийный срок составляет 12 месяцев со дня подписания акта окончательной приёмки работ.")]),
        P([T("5. РЕКВИЗИТЫ И ПОДПИСИ СТОРОН",{b:true})]),
        P([]),
        sigTable(),
        ...annex1,
      ];
      ...
      children = [
        ...
        PC([T("к Договору ремонтно-отделочных работ",{sz:12,b:true})]),
        PC([T("\u2116"+(c.mainNumber||"___")+" от \u00ab"+dtM.d+"\u00bb "+dtM.m+" "+dtM.y+" г.")]),
        PC([T(dtA.full+" г.          г. Караганда")]),
        P([]),
        P([T("Дополнительный перечень работ",{b:true,sz:12})],{al:"center"}),
        P([]),
        ...preamParas("Подрядчик"),
        P([]),
        makeWorksTable(),
      ...
      children = [
        PC([T("Договор \u2116"+(c.number||"___"),{sz:13,b:true})]),
        PC([T("на разработку дизайн-проекта",{sz:12,b:true})]),
        PC([T(dt.full+" г.          г. Караганда")]),
        P([]),
        ...preamParas("Исполнитель"),
        P([]),
        P([T("1. ПРЕДМЕТ ДОГОВОРА",{b:true})]),
        P([T("1.1. Исполнитель обязуется разработать дизайн-проект объекта Заказчика, а Заказчик обязуется принять и оплатить результат.")]),
        P([T("2. СТОИМОСТЬ И ПОРЯДОК ОПЛАТЫ",{b:true})]),
        P([T("2.1. Стоимость работ определяется по результатам обмера объекта и согласования технического задания.")]),
        P([T("2.2. Предоплата при заключении договора: "+fmtN2(advD)+" \u20b8. При расторжении договора предоплата возврату не подлежит.")]),
        P([T("3. СРОКИ",{b:true})]),
        P([T("3.1. Сроки разработки дизайн-проекта определяются дополнительно после согласования технического задания.")]),
        P([T("4. РЕКВИЗИТЫ И ПОДПИСИ СТОРОН",{b:true})]),
        P([]),
        sigTable(),
      ];
    } else if(type==="design_add"){
      const advD = c.designAdvance||25000;
      const area = c.area||"___";
      const deadline = c.deadline||"___";
      const priceType = c.priceType||"fixed";
      const totalCost = priceType==="sqm"?(Number(c.pricePerSqm||0)*Number(area||0)):Number(c.totalCost||0);
      children = [
        PC([T("Дополнительное соглашение \u2116"+(c.number||"___"),{sz:13,b:true})]),
        PC([T("к Договору на разработку дизайн-проекта",{sz:12,b:true})]),
        PC([T(dt.full+" г.          г. Караганда")]),
        P([]),
        ...preamParas("Исполнитель"),
        P([]),
        P([T("1. ПРЕДМЕТ",{b:true})]),
        P([T("1.1. Стороны договорились о следующих условиях разработки дизайн-проекта:")]),
        P([T("- Площадь объекта: "+area+" м\u00b2")]),
        P([T("- Срок разработки: "+deadline)]),
        P([T("- Стоимость: "+fmtN2(totalCost)+" \u20b8"+(priceType==="sqm"?" ("+fmtN2(c.pricePerSqm||0)+" \u20b8/м\u00b2)":""))]),
        P([T("2. ОПЛАТА",{b:true})]),
        P([T("2.1. Предоплата: "+fmtN2(advD)+" \u20b8.")]),
        P([T("2.2. Оставшаяся сумма оплачивается по факту выполнения работ.")]),
        P([T("3. РЕКВИЗИТЫ И ПОДПИСИ СТОРОН",{b:true})]),
        P([]),
        sigTable(),
      ];
    } else if(type==="reservation"){
      const resAmt = c.reserveAmount||50000;
      const resDate = fmtDate(c.reserveStartDate||c.date);
      children = [
        PC([T("Соглашение о бронировании \u2116"+(c.number||"___"),{sz:13,b:true})]),
        PC([T(dt.full+" г.          г. Караганда")]),
        P([]),
        ...preamParas("Исполнитель"),
        P([]),
        P([T("1. ПРЕДМЕТ",{b:true})]),
        P([T("1.1. Заказчик вносит бронь в размере "+fmtN2(resAmt)+" \u20b8 для резервирования даты начала работ.")]),
        P([T("2. УСЛОВИЯ",{b:true})]),
        P([T("2.1. Бронь вносится с "+resDate.d+" "+resDate.m+" "+resDate.y+" г. и засчитывается в счёт основного договора.")]),
        P([T("2.2. В случае отказа Заказчика от заключения основного договора сумма брони возврату не подлежит.")]),
        P([T("3. РЕКВИЗИТЫ И ПОДПИСИ СТОРОН",{b:true})]),
        P([]),
        sigTable(),
      ];
