...
// словах — он выбирает стадию съёмки.
//
// КЛИЕНТУ ПИШЕМ ОБА СЛОВА, И ЭТО НЕ ПРИДИРКА К ФОРМУЛИРОВКЕ. Сначала здесь
// стояло только «Скрытые работы»: «В процессе» само по себе клиенту ничего не
// объясняет, ему важно, ЧТО снимок доказывает. Но под этой подписью уезжает ВСЁ,
// что снято по ходу ремонта, — и стяжка, и разводка, и просто «вот так сейчас
// выглядит комната». Называть это всё скрытыми работами неверно: владелец
// смотрел на свои же три снимка и увидел заголовок, который к ним подходит
// наполовину. Поэтому подпись честно называет оба смысла.
export const PHOTO_KINDS = [
  { key: "before", label: "До", hint: "как было до работы",
    clientLabel: "Как было", clientHint: "до начала работ" },
  { key: "during", label: "В процессе", hint: "ход работ и скрытое: каркас, утеплитель, разводка",
    clientLabel: "Скрытые работы · в процессе",
    clientHint: "как шёл ремонт, включая каркас, утеплитель и разводку — после отделки этого уже не увидеть" },
