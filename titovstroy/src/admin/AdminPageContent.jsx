...
const ROLE_PERMISSION_ACTIONS = ROLE_PERMISSION_GROUPS.flatMap(group => group.actions);
...
export default function AdminPageContent({ currentUser, presence = {}, permissions=DEFAULT_ROLE_PERMISSIONS.admin, onUsersChanged, rolePermissions=DEFAULT_ROLE_PERMISSIONS, onSaveRolePermissions=async()=>false, clients=[], saveClients=()=>{}, clientsRef={current:[]}, contragents=[], saveContragents=()=>{}, contragentsRef={current:[]}, workers=[], saveWorkers=()=>{}, workersRef={current:[]}, contracts=[], documentTemplateEnabled=false, documentTemplateService=null, documentTemplateData={}, fmt=(n)=>Math.round(Number(n)||0).toLocaleString("ru-RU"), onBackupWorkspace=()=>{}, onExportAll=()=>{}, onImportAll=()=>{}, onExportEstimatesXls=()=>{}, checkIssues=[], onNavIssue=()=>{}, runtime }) {
  ...
  const renameWork = async (code, newName) => { const cur = getCatalogOverrides(); await saveCatalog(withCatalogOverrides(cur, { renames: { ...(cur.renames||{}), [code]: newName } })); };
  ...
  const renameCat = async (origKey, newCat) => {
    ...
    const cur = getCatalogOverrides(); const cr = { ...(cur.catRenames||{}), [origKey]: newCat.trim() }; const currentName = (cur.catRenames||{})[origKey] || origKey;
  ...
  const renameSub = async (origCatKey, origSubKey, newSub) => {
    ...
    const cur = getCatalogOverrides(); const key = origCatKey+"|"+origSubKey; const sr = { ...(cur.subRenames||{}), [key]: newSub.trim() };
