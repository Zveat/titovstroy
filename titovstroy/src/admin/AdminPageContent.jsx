...
export function AdminPageContent({ currentUser, presence = {}, onAuditPrice = null, permissions=DEFAULT_ROLE_PERMISSIONS.admin, onUsersChanged, rolePermissions=DEFAULT_ROLE_PERMISSIONS, onSaveRolePermissions=async()=>false, clients=[], saveClients=()=>{}, clientsRef={current:[]}, contragents=[], saveContragents=()=>{}, contragentsRef={current:[]}, workers=[], saveWorkers=()=>{}, workersRef={current:[]}, contracts=[], documentTemplateEnabled=false, documentTemplateService=null, documentTemplateData={}, fmt=(n)=>Math.round(Number(n)||0).toLocaleString("ru-RU"), onBeforePriceChange=async()=>true, onBackupWorkspace=()=>{}, onExportAll=()=>{}, onImportAll=()=>{}, onExportEstimatesXls=()=>{}, checkIssues=[], onNavIssue=()=>{} }) {
  ...
  const removeUser = async (id) => {
    ...
    // СПРАШИВАЕМ. Раньше крестик удалял учётку сразу, с первого нажатия, и стоит он вплотную
    // к «Разлогинить» — то есть промах мимо соседней кнопки стоил человеку доступа. Отмены у
    // этого действия нет: вернуть учётку можно только заведя заново и назначив пароль и права.
    // Имя в вопросе не для красоты: в списке карточки идут вплотную, и нажать на соседнюю
    // строку легко — по имени сразу видно, что удаляешь не того.
    if (!confirmDangerous(`Удалить сотрудника: ${gone?.name || gone?.login || id}?\n\nОн потеряет доступ в систему. Отменить это нельзя — учётную запись придётся заводить заново.\n\nЕсли нужно просто закрыть ему сессию — нажмите «Разлогинить».`)) return;
  ...
  return (
    <div className="page" style={{maxWidth:1600}}>
      ...
      {/* Табы. Одной компактной строкой — так и было, и так правильно: панель
          не должна занимать четверть экрана. Ушли только два изъяна: flex:1
          растягивал кнопки на всю ширину независимо от длины подписи, а
          overflow-x давал полосу прокрутки. Теперь ширина по содержимому и
          перенос на вторую строку, если не помещается. */}
      <style>{`
        .admt { border:1px solid transparent; background:transparent; color:#64748b;
                border-radius:8px; padding:9px 13px; font:inherit; font-size:12px;
                font-weight:700; cursor:pointer; white-space:nowrap; transition:all .12s }
