...
export default function ObjectDispatcher({ objects = [], productions = [], onOpenObject, open = false, onToggle }) {
  const rows = useMemo(
  ...
  return (
    <section style={{ margin: "2px 0 4px", background: "#fff", border: "1px solid #fecaca", borderRadius: 8, overflow: "hidden" }}>
      <button type="button" onClick={() => onToggle?.(!open)}
        ...
        <span style={{ fontSize: 10.5, color: "#94a3b8", whiteSpace: "nowrap" }}>{open ? "▲" : "▼"}</span>
      </button>

      {open && riskyRows.map((row) => (
