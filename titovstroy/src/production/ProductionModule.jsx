...
//   buildStagesFromEstimate(objectId) — вернуть этапы из сметы [{name, priceClient, costPlan}]
...
export default function ProductionModule({
  ...
  // Прогресс чек-листа запуска (для индикатора в списке)
  ...
  // патч production-карточки + сохранение
  const patchProd = (patch) => {
    const next = { ...openProd, ...patch, updatedAt: Date.now() };
    onSaveProduction(next);
  };
  ...
  return (
    <div style={{ maxWidth: 1100, margin: "0 auto", padding: "0 4px" }}>
      ...
      {/* Табы */}
      <div style={{ display: "flex", gap: 4, marginBottom: 18, flexWrap: "wrap", borderBottom: "1px solid #e2e8f0", paddingBottom: 0 }}>
        {TABS.map(t => (
...
// ─── ВКЛАДКА: ЭТАПЫ И СРОКИ (+ мини-Gantt) ───
function StagesTab({ prod, patch, genId, fmt, buildStagesFromEstimate, objId }) {
  ...
  const add = () => patch({ stages: [...stages, { id: genId(), name: "", planStart: "", planEnd: "", factStart: "", factEnd: "", status: "todo", responsible: "", note: "" }] });
  const fillFromEstimate = () => {
    const fromEst = buildStagesFromEstimate(objId);
    if (!fromEst.length) { alert("Нет привязанной сметы с позициями для автозаполнения."); return; }
    const existNames = new Set(stages.map(s => (s.name || "").toLowerCase()));
    const toAdd = fromEst
      .filter(s => !existNames.has((s.name || "").toLowerCase()))
      .map(s => ({ id: genId(), name: s.name, planStart: "", planEnd: "", factStart: "", factEnd: "", status: "todo", responsible: "", note: "", priceClient: s.priceClient || 0, costPlan: s.costPlan || 0 }));
  ...
  const barStyle = (start, end, color) => {
    if (!start || !end) return null;
    const s = new Date(start).getTime(), e = new Date(end).getTime();
    const left = ((s - minD) / span) * 100;
    const width = Math.max(2, ((e - s) / span) * 100);
    return { position: "absolute", left: `${left}%`, width: `${width}%`, height: 7, borderRadius: 4, background: color, top: "50%", transform: "translateY(-50%)" };
  };
      ...
      <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 14, padding: 18 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14, flexWrap: "wrap", gap: 8 }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={fillFromEstimate} style={{ background: "#ecfdf5", color: "#059669", border: "1px solid #bbf7d0", borderRadius: 8, padding: "7px 14px", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>↧ Заполнить из сметы</button>
            <button onClick={add} style={{ background: "#2563eb", color: "#fff", border: "none", borderRadius: 8, padding: "7px 14px", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>+ Этап</button>
          </div>
        </div>
        {stages.length === 0 && <div style={{ textAlign: "center", color: "#94a3b8", padding: "30px 0", fontSize: 13 }}>Нет этапов. Заполните из сметы или добавьте вручную.</div>}
        ...
        <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 14, padding: 18, overflowX: "auto" }}>
          ...
          {stages.filter(s => s.name).map(s => (
            <div key={s.id} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
              <div style={{ width: 160, fontSize: 12, color: "#0f172a", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", flexShrink: 0 }}>{s.name}</div>
              <div style={{ position: "relative", flex: 1, height: 24, background: "#f8fafc", borderRadius: 6, minWidth: 200 }}>
                {barStyle(s.planStart, s.planEnd, "#93c5fd") && <div style={{ ...barStyle(s.planStart, s.planEnd, "#93c5fd"), top: 6, transform: "none" }} />}
                {barStyle(s.factStart, s.factEnd, "#059669") && <div style={{ ...barStyle(s.factStart, s.factEnd, "#059669"), top: 13, transform: "none" }} />}
              </div>
            </div>
          ))}
        </div>
...
// ─── ВКЛАДКА: ФИНАНСОВЫЙ БЛОК (план/факт по этапам, маржа) ───
function FinanceTab({ prod, patch, genId, fmt, buildStagesFromEstimate, objId }) {
  ...
  const fillFromEstimate = () => {
    ...
    const toAdd = fromEst
      .filter(s => !existNames.has((s.name || "").toLowerCase()))
      .map(s => ({ id: genId(), name: s.name, planStart: "", planEnd: "", factStart: "", factEnd: "", status: "todo", responsible: "", note: "", priceClient: s.priceClient || 0, costPlan: s.costPlan || 0 }));
    // также обновляем план-цифры для уже существующих по совпадению имени
    const merged = stages.map(s => {
      const m = fromEst.find(f => (f.name || "").toLowerCase() === (s.name || "").toLowerCase());
      return m ? { ...s, priceClient: s.priceClient || m.priceClient || 0, costPlan: s.costPlan || m.costPlan || 0 } : s;
    });
    patch({ stages: [...merged, ...toAdd] });
  };
  const num = (v) => Number(v) || 0;
  const tot = stages.reduce((a, s) => {
    a.priceClient += num(s.priceClient);
    a.costPlan += num(s.costPlan);
    a.costFact += num(s.costFact);
    return a;
  }, { priceClient: 0, costPlan: 0, costFact: 0 });
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 14, padding: 18, overflowX: "auto" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
          ...
          <button onClick={fillFromEstimate} style={{ background: "#ecfdf5", color: "#059669", border: "1px solid #bbf7d0", borderRadius: 8, padding: "7px 14px", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>↧ Загрузить план из сметы</button>
        </div>
        {stages.length === 0 ? (
          <div style={{ textAlign: "center", color: "#94a3b8", padding: "30px 0", fontSize: 13 }}>Нет этапов. Добавьте этапы во вкладке «Этапы» или загрузите из сметы.</div>
        ) : (
          <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 600 }}>
            ...
            <tbody>
              {stages.map(s => {
                ...
                return (
                  <tr key={s.id}>
                    <td style={{ ...cell, textAlign: "left", fontWeight: 600, color: "#0f172a" }}>{s.name || "—"}</td>
