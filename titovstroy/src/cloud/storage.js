...
export const storage = {
  ...
  // ── ОТМЕТКА «БЫЛ В СЕТИ» — МИМО ЗАМКА РЕДАКТОРА ──
  //
  // НАБЛЮДАТЕЛЬ НЕ ЗАХВАТЫВАЕТ ЗАМОК ВООБЩЕ (EditorSessionGate: роль viewer сразу уходит в
  // readonly), но _leaseEnforced ему выставляется. Значит центральный гейт _writeGateFail
  // отбивает у него ЛЮБУЮ запись — и служебный heartbeat присутствия в том числе. Наружу это
  // вылезало так: в журнале видно, что человек заходил сегодня трижды, а на карточке
  // сотрудника «был в сети 07.09» — то есть с того дня, когда ему поменяли роль. Вход в
  // журнал пишется ДО монтирования оболочки, пока гейт ещё не включён, поэтому он-то на месте.
  // Ровно то же било по второй вкладке любого сотрудника: замок у первой, у второй присутствие
  // не обновлялось.
  //
  // Правила базы это давно разрешают (ветка ro пускает titovstroy_audit и titovstroy_presence
  // именно для этого) — упиралось всё в клиентский гейт.
  //
  // Дыра узкая и намеренная: только СВОЙ ключ присутствия, только облако, без localStorage и
  // без dirty-метки. Это не бизнес-данные, а отметка «я тут», у каждого своя; перетереть ею
  // чужую работу нельзя. Имя ключа проверяется здесь же — метод не годится ни для чего другого.
  async setPresence(key, value) {
    if (!/^titovstroy-presence-[A-Za-z0-9_-]+$/.test(String(key))) {
      return { fbOk: false, fbError: "not-a-presence-key" };
    }
    try {
      if (_fbDb) {
        await _fbAuthReady;
