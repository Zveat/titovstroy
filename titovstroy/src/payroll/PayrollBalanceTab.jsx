...
export function BalanceTab({ staff, accruals, report, money }) {
  ...
  if (bal.rows.length === 0) return (
    <div style={{ ...card, fontSize: 13, color: C.mute2, lineHeight: 1.55 }}>
                  ...
                  {months.map(m => {
                    ...
                    return (
                      <td key={m} style={numCell}>
                        ...
                        <div style={{ fontSize: 11, color: c.delta > 0 ? C.red : C.faint, marginTop: 2 }}>выпл. {money(c.paid)}</div>
                      </td>
                    );
                  })}
                  <td style={{ ...numCell, fontWeight: 800, fontSize: 14, color: C.ink }}>{money(r.accrued)}</td>
                  <td style={{ ...numCell, color: C.mute }}>{money(r.paid)}</td>
                  <td style={{ ...numCell, fontWeight: 800, fontSize: 13.5, color: r.balance > 0 ? C.red : r.balance < 0 ? C.warnText : C.green }}>
