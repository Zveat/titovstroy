...
//
// КТО СЮДА ХОДИТ:
//   • планировщик Vercel (vercel.json → crons). Он сам подставляет заголовок
//     Authorization: Bearer <CRON_SECRET> из переменных окружения проекта.
//     На тарифе Hobby он срабатывает РАЗ В СУТКИ и с точностью «в течение часа»,
//     поэтому как единственный источник расписания не годится;
//   • внешний планировщик — тот же, что уже дёргает парсер мастеров каждые
//     полчаса. Ему нужен тот же заголовок с секретом. Вот он даёт точное время:
//     сводка, назначенная на 10:00, уходит в 10:00.
//
