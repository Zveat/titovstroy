...
// ── КОГДА БЫЛ ПРЕДЫДУЩИЙ ЗАХОД ────────────────────────────────────────────────
//
// Расписание всех парсеров считается от lastRunAt, а он пишется В КОНЦЕ прогона, то есть
// ТОЛЬКО ПОСЛЕ УСПЕХА. Пока прогоны заканчивались успехом, это работало. Как только прогон
// начал падать, lastRunAt перестал двигаться — и каждый следующий заход внешнего крона
// (а он дёргает воркфлоу каждые полчаса) снова видел «пора» и снова читал базу мастеров
// целиком, чтобы упасть на том же месте.
//
// Так и вышло: 7 сентября узел OLX перерос предел Firebase в 10 МБ на одно значение, запись
// стала падать, и вместо двух полных обходов в сутки парсер сделал 48 — каждый с чтением
// десяти мегабайт. Почти полгигабайта в день за работу, которая никуда не сохранялась.
//
// Поэтому «когда был предыдущий заход» — это ПОПЫТКА, а не успех. Отметку lastTryAt парсер
// ставит ДО дорогой части; упал он или нет, следующий заход придёт по обычному расписанию.
// Кнопка «Обновить сейчас» работает как работала: она идёт по runNow, мимо этого зазора.
...
// Список растёт сам: mergeFreshSnapshot ничего не выбрасывает, пропавшие объявления просто
// помечаются active:false и остаются навсегда. 7 сентября OLX упёрся в этот предел, и
// firebase-admin ответил английской строкой про «string greater than 10485760 utf8 bytes»
// со стеком внутри своей библиотеки — по такому логу не понять ни что случилось, ни что делать.
//
// Проверяем размер сами и говорим по-русски: сколько получилось, из чего состоит и какие есть
// выходы. Запись при этом не делаем — молча потерять часть списка хуже, чем упасть.
export const FB_VALUE_LIMIT = 10 * 1024 * 1024;

export function assertWritableSize(key, json, { limit = FB_VALUE_LIMIT, items = null } = {}) {
  const size = Buffer.byteLength(json, "utf8");
  if (size <= limit) return size;
  const mb = (n) => (n / 1024 / 1024).toFixed(2) + " МБ";
  const list = Array.isArray(items) ? items : null;
  const parts = list
    ? ` Записей ${list.length}: активных ${list.filter((m) => m?.active !== false).length},`
      + ` пропавших ${list.filter((m) => m?.active === false).length},`
      + ` с телефоном ${list.filter((m) => m?.phone).length}.`
    : "";
  throw new Error(
    `${key}: не помещается в базу — ${mb(size)} при пределе ${mb(limit)} на одно значение.${parts}`
    + " Список копится без конца: пропавшие объявления остаются в нём навсегда."
    + " Нужно решение владельца — чистить историю или хранить список по частям."
    + " Ничего не записано, старые данные на месте.",
  );
}
