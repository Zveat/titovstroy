...
// ПОСЛЕ ПЕРВОГО ЖЕ БАНА УХОДИМ. Было 3 — то есть две паузы по 150 секунд «а вдруг
// отпустит». Снято по логам трёх потоков за два прогона: не отпускает НИ РАЗУ.
//   09:00 поток 0: 19 номеров до бана, после двух пауз — 0
//   10:00 поток 1: 19 номеров до бана, после двух пауз — 0
//   10:00 поток 2:  2 номера до бана, после двух пауз — 0
// Пять минут сна на поток ради нуля. Заход теперь длится 2-3 минуты вместо восьми,
// а собирает столько же. Терять нечего и в худшем случае: мастера при бане не
// помечаются, он остаётся в очереди и достаётся следующему заходу через полчаса —
  ...
  for (let idx = 0; idx < targets.length; idx++) {
    ...
    if (plan.action !== "accept") {
      ...
      if (plan.action === "giveup") {
        console.warn(throttleStreak === 1
          ? `  ! OLX закрыл выдачу — стоп до следующего прогона (собрано ${gotPhones}, остальные вернутся в очередь)`
          : `  ! OLX держит лимит ${throttleStreak} раз подряд — стоп до следующего прогона (собрано ${gotPhones})`);
        break;
      }
