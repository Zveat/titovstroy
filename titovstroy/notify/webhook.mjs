...
async function main() {
  ...
  if (ACTION === "включить") {
    ...
    // Проверяем ДО обращения к Telegram. Иначе в логе появляется его английское
    // «secret token contains illegal characters» без единой подсказки, что
    // делать. Сам секрет в лог не выводим — только сколько символов не подходит:
    // чаще всего это кириллица, набранная в русской раскладке.
    const bad = SECRET.replace(/[A-Za-z0-9_-]/g, "").length;
    if (bad || SECRET.length > 256) {
      throw new Error(`Секрет не подходит Telegram: ${bad ? `${bad} недопустимых символов` : "длиннее 256 символов"}. `
        + "Разрешены ТОЛЬКО латинские буквы, цифры, «_» и «-» — ни кириллицы, ни "
        + "пробелов, ни знаков. Скорее всего набрано в русской раскладке. "
        + "Замените значение в ДВУХ местах сразу: GitHub → Secrets → Actions и "
        + "Vercel → Settings → Environment Variables, и передеплойте сайт.");
    }
